                ...
                <van-cell v-for="item in list" :key="item" :title="item.title" title-style="text-align: left; margin-left: 20px" :icon="item.avatar" @click="onClickUser(item.id)">
                    <template #icon>
                        <van-image
                                ...
                                width="45"
                                height="45"
                                src="http://imagemxc.test.upcdn.net//用户.png"
                        >
                        </van-image>
                    </template>
                    <template #label>
...
                />
        ...
        <van-popup v-model="showDeletePanel" round style="height: 200px; width: 300px">
            <div style="margin-top: 40px; margin-bottom: 20px">您确认要删除该用户吗？</div>
...
import axios from 'axios';
    export default {
        ...
        methods: {
            ...
            toUserHome(userId){
                this.$router.push({ path: "/CS/userHomeSP/" + userId });
            }
        }
    }
</script>
