...
class WorkOrderRet extends Ret{
	...
	public void addWorkOrder(WorkOrder o) {
		...
		if(o.getPictures().size() > 0)
			this.workorderpic.add(Execute.getFileContent(o.getPictures().get(0)) );
...
class OrdersRet extends RoomRet{
	...
	private ArrayList<Object> serviceId;
	...
	public void addOrders(Orders o) {
		...
		contracturl.add(o.getOrdersContract() );
	}
	...
	public void addAllOrders(ArrayList<Orders> o) {
		...
		this.address = new ArrayList<>();
		for(Orders oo:o) {
			this.address.add(Room.findRoomById(oo.getRoomId()).getAddress());
		}
	}
	...
	public ArrayList<Object> getServiceId() {
		return serviceId;
	}

	public void setServiceId(ArrayList<Object> serviceId) {
		this.serviceId = serviceId;
	}
...
public class Controller {
	...
	public Object renterHomePage() {
		...
		while(rooms.size() < 8) {
			...
			if(rm!=null && !rooms.contains(rm)&&rm.getState().equals("available") ) {
				rooms.add(rm);
			}
			i++;
			if(i>100)
	...
	public Object renterComplaint(@RequestParam(value = "roomid") String rid,
		...
		if(type.equals("room")) {
			...
			for(int i=0; i<pictures.length; i++) {
				if(i%2 == 0) { // 图片名
					code = "data:" + Execute.getFileForm(pictures[i]) + ";base64,";
					name = "workorder" + wo.getId() + String.valueOf(i/2+1) + ".txt";
					
					System.out.println("name:" + name);
				}
				else{ // 图片内容
					code += pictures[i];
					if(Execute.writeFile(name,code) == false) {
						System.out.println("false");
						return i/2+1;
					}
					wo.addPictures("./pictures/" + name);
				}
			}
			...
			for(int i=0; i<pictures.length; i++) {
				if(i%2 == 0) { // 图片名
					code = "data:" + Execute.getFileForm(pictures[i]) + ";base64,";
					name = "complaint" + c.getId() + String.valueOf(i/2+1) + ".txt";
				}
				else{ // 图片内容
					code += pictures[i];
					if(Execute.writeFile(name,code) == false) {
	...
	public Object serviceAddRoom(@RequestParam("RoomType") String roomtype,
		...
		r.setSuccess(Room.addNewRoom(new Room(id,serviceId,roomtype,roomavailable,roomrent,roomaddress,roomarea,roomitem,id + "01.jpg")));
		System.out.println(r.isSuccess());
		return r;
	}
	...
	/*
	 ...
	 public Object workOrderInitial(@RequestParam("WorkOrderId") String id) {
	  ...
	  if(rt.getPictures().size()>0)
		  r.addObj(Execute.getFileContent(rt.getPictures().get(0)) );
	 r.setSuccess(true);
	  return r;
	 }
	  ...
	  * 客服帮助房间
	  */
	 @RequestMapping(value = "/srentroom")
	 @ResponseBody
	 @CrossOrigin
	 public Object srentRoom(@RequestParam("RoomId") String rid,
			 @RequestParam("UserId") String uid,
			 @RequestParam("BookType") String type,
			 @RequestParam("BookComment") String time) {
	  System.out.println("modify room");
	  Ret r=new Ret();
	  Room c=Room.findRoomById(rid);
	  String id=Orders.getNextId();
	  Orders o = new Orders(id,uid,rid,type,time,c.getRent());
	  Orders.addNewOrders(o);
	  Renter.findRenterById(uid).payForRoom(rid,type,time);
	  r.setSuccess(true);
	  return r;
	 }
	 /*
	 ...
	 public Object rmReplyList(@RequestParam("Rid") String id) {
	  ...
	  for(int i=0;i<w.size();i++) {
	   r.addObj(new rmReply(w.get(i).getId(),w.get(i).getRenterId(),w.get(i).getItem(),w.get(i).getRepairmanItem(),w.get(i).getRoomId()));
	  }
	  r.setSuccess(true);
	  return r;
	 }
	 class rmReply{
		 public String ReportID,ReportUser,RoomId,ReportItem,replied;
		 public rmReply(String rid,String ruser,String ritem,String replied,String room) {
			this.ReportID=rid;
	 ...
	 public Object rmList(@RequestParam("Rid") String id) {
		  ...
		  ArrayList<WorkOrder> w=WorkOrder.findWorkOrderByRepairmanId(id);
		  for(int i=0;i<w.size();i++) {
		   r.addObj(new rm(w.get(i).getId(),w.get(i).getRenterId(),w.get(i).getRoomId(),w.get(i).getItem(),w.get(i).getState()));
		  }
		  r.setSuccess(true);
		  return r;
		 }
		 class rm{
			 public String Repair_jobID,Repair_jobUser,RoomId,Repair_jobItem,replied;
			 public rm(String rid,String ruser,String room,String ritem,String replied) {
